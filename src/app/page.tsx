                ...
                return (
                  <Link href={`/product/${book.slug}`} key={book._id} className="group flex flex-col">
                    ...
                    <div className="flex items-center gap-2 md:gap-3 mt-2">
                      ...
                      <span className="text-zinc-300 line-through text-[10px] md:text-xs">{book.price} RON</span>
                    </div>
                  </Link>
                );
              })}
      ...
      <section className="py-20 md:py-32">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20 md:mb-32">
            <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.6em] text-zinc-400 mb-6">Cele mai citite</p>
  ...
  {restOfTop.map((book: any, index: number) => (
    <Link href={`/product/${book.slug}`} key={book._id} className="group flex gap-6 items-center text-left relative">
      ...
      <div className="flex flex-col justify-center">
        ...
        <p className="text-[9px] uppercase tracking-[0.2em] text-zinc-400 mt-1 font-bold italic">
          {book.authorName}
        </p>
      </div>
    </Link>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-16 gap-6">
            <div className="text-left">
              <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] text-zinc-400 mb-2 italic">Proaspăt tipărite</p>
            ...
            <Link href="/noutati" className="flex items-center gap-3 text-zinc-400 uppercase tracking-[0.3em] text-[9px] md:text-[10px] font-bold hover:text-zinc-900 transition-all group">
            ...
            {data.newArrivals.map((book: any) => (
              <Link href={`/product/${book.slug}`} key={book._id} className="group flex flex-col">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[8px] md:text-[9px] uppercase tracking-widest text-zinc-400 mt-2 font-bold italic">{book.authorName}</p>
                </div>
              </Link>
            ))}
          </div>
  ...
  return (
    <Link href={`/product/${book.slug}`} className="group relative block text-center">
      ...
      <div className="mt-10 px-2 text-left md:text-center">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400 mt-3 italic">
          {book.authorName}
        </p>
      </div>
    </Link>
