  ...
  return (
    <div className="space-y-20">
      ...
      <div className="relative border-b-2 border-zinc-900 pb-4 group">
        <Search className="absolute left-0 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-zinc-900 transition-colors" size={32} strokeWidth={1} />
        <input
          type="text"
          placeholder="Caută titlu, autor sau gen..."
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          className="w-full pl-12 text-2xl md:text-4xl font-playfair italic bg-transparent outline-none placeholder:text-zinc-500 text-zinc-900"
          autoFocus
        />
        {query && (
          <button onClick={() => setQuery('')} className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-900">
            <X size={24} />
          </button>
        )}
      </div>
      ...
      {query.length > 0 && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-12">
            ...
            {filteredBooks.map((book) => (
              <Link href={`/product/${book.slug}`} key={book._id} className="group flex flex-col">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[9px] uppercase tracking-widest text-zinc-500 mt-2 font-bold italic">
                    {book.authorName}
                  </p>
      ...
      {query.length > 0 && filteredBooks.length === 0 && (
        <div className="py-20 text-center">
          <p className="font-playfair text-xl italic text-zinc-500">Nu am găsit nicio potrivire pentru "{query}"</p>
        </div>
      )}
    </div>
  );
